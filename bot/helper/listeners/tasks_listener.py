...
from bot.helper.ext_utils.ffmpeg import edit_attachment
...
class MirrorLeechListener:
    ...
    def __setModeEng(self):
        mode = f" #{'🚀 Leech' if self.isLeech else '♻️ Clone' if self.isClone else '☁️ RClone' if self.upPath not in ['gd', 'ddl'] else '📎 DDL' if self.upPath != 'gd' else '🧩 GDrive'}"
        mode += " (🗜️ Zip)" if self.compress else " (📂 Unzip)" if self.extract else ""
    ...
    async def onDownloadStart(self):
        ...
        if self.isPM and self.isSuperGroup:
            self.botpmmsg = await sendCustomMsg(
                self.message.from_user.id, BotTheme("PM_START", msg_link=self.source_url)
            )
    ...
    async def onDownloadComplete(self):
        ...
        if self.compress:
            ...
            if self.isLeech and int(size) > LEECH_SPLIT_SIZE:
                if not pswd:
                    del cmd[4]
                LOGGER.info(f"🗜️ Zip: orig_path: {dl_path}, zip_path: {up_path}.0*")
            else:
                del cmd[1]
                if not pswd:
                    del cmd[3]
                LOGGER.info(f"🗜️ Zip: orig_path: {dl_path}, zip_path: {up_path}")
            if self.suproc == "cancelled":
                return
            self.suproc = await create_subprocess_exec(*cmd)
            code = await self.suproc.wait()
        ...
        if self.isLeech:
            ...
            size = await get_path_size(up_path)
            LOGGER.info(f"☁️ Upload Name: {up_name} via RClone")
        ...
        if self.isLeech:
            ...
            if config_dict["MIRROR_LOG_ID"] and not self.excep_chat:
                ...
                if self.linkslogmsg:
                    ...
                    await editMessage(
                        self.linkslogmsg,
                        (msg + BotTheme("LINKS_SOURCE", On=dispTime, Source=self.source_msg) + BotTheme("L_LL_MSG") + f"\n\n<a href='{log_msg.link}'>{escape(name)}</a>\n"),
                        _btns.build_menu(1),
                    )
...
🏷️ <b>Task for:</b> {self.tag}

🚫 <b>Due To:</b> {escape(error)}
⚙️ <b>Mode:</b> {self.upload_details['mode']}
⏱️ <b>Elapsed:</b> {get_readable_time(time() - self.message.date.timestamp())}"""
