...
async def clean_target(path):
    if await aiopath.exists(path):
        LOGGER.info(f"🗑️ Cleaning Target: {path}")
...
async def clean_download(path):
    if await aiopath.exists(path):
        LOGGER.info(f"🗑️ Cleaning Download: {path}")
        try:
            await aiormtree(path)
        except Exception:
            pass
...
def exit_clean_up(signal, frame):
    try:
        ...
        LOGGER.warning("⚠️ Force Exiting before the cleanup finishes!")
        sexit(1)


async def clean_unwanted(path):
    LOGGER.info(f"♻️ Cleaning unwanted files/folders: {path}")
    for dirpath, _, files in await sync_to_async(walk, path, topdown=False):
        for file_ in files:
            if (
                file_.endswith(".!qB")
                or file_.endswith(".parts")
                and file_.startswith(".")
            ):
...
async def join_files(path):
    ...
    for file_ in files:
        if (
            ...
            LOGGER.warning("⚠️ No Binary files to join!")
    if results:
